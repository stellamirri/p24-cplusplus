...
void generate_apple(std::pair<int, int> pomme, std:: vector<std::pair<int, int>> pos_snake) {     // qui génère une pomme au hasard sur le board.
    ...
    // faire en sorte que la pomme soit pas placée sur une des positions du serpent :
    bool touch_snake = false; // on initialise à false le fait que la pomme touche le serpent.
    // le serpent est un vecteur de points : il faut donc vérifier qu'aucun de ces points (déf comme une paire de coordonnées) ne touche la pomme d'où la présence du "for"
